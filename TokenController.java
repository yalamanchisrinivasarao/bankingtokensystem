...
public class TokenController {
    ...
    public Integer create(@RequestBody @NotNull @Valid TokenRequest tokenRequest) {
        Customer customer = customerController.findByMobile(tokenRequest.getCustomerMobile());
        if (customer == null) {
        ...
        for (String s : tokenRequest.getServices()) {
            Service service = serviceController.findByName(s);
            if (service == null) {
            ...
            while (service.getNextServiceId() != null) {
                Service nextService = serviceController.findById(service.getNextServiceId());
        ...
        counterController.incrementQueueSize(counter.getId());
    ...
    public void cancel(@PathVariable("tokenNumber") @NotNull Integer tokenNumber) {
        ...
        counterController.decrmentQueueSize(counter.getId());
